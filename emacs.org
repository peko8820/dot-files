...
** COMMENT Emoji support
   ...
   Although pure text files are great for my calendar exports and
   various other occasions an Emoji here and there is a handy thing to
   have. There are a few ways of inserting these little images. For
   once there is the [[https://github.com/iqbalansari/emacs-emojify][emojify]] project. On the other hand there is the
   [[https://github.com/syl20bnr/emacs-emoji-cheat-sheet-plus][emacs-emoji-cheat-sheet-plus]] project which offers very nice helm
   support but does not render ascii emojis.
   ...
   For right now I will stick with the emojify project especially
   because when turned on it will render emojis automatically.
** Calendar Tweeks
...
** Magit
   ...
   #+BEGIN_SRC elisp
     (use-package magit
       ...
       (when (and (eq system-type 'windows-nt) (string= user-login-name 'sieversm))
         (setenv "SSH_AGENT_PID" "1032")
         (setenv "SSH_AUTH_SOCK" "C:/Users/sieversm/home/.ssh-socket"))
   ...
   #+BEGIN_SRC elisp
     (use-package magit-gerrit
       :ensure t
       :config
       (setq magit-gerrit-ssh-creds "sieversm@gerrit.intra.infineon.com"))
   #+END_SRC
