...
** Loading this File
   ...
   Because at work we have to tunnel through the proxy and want it to
   automatically check where I am using Emacs the following
   lines need to be added to our =$HOME/.emacs= as well:

   #+BEGIN_SRC elisp :tangle no
     ;; Make sure that we can get through the proxy at work, 
     ;; but only at work
     (if (string= user-login-name '<log-in-name>)
...
** Setting up the Load Path
   ...
   #+BEGIN_SRC 
     ;; Add the require statements for the personalized .el files
     (require 'cl)
   #+END_SRC
...
* Variables
  ...
  #+BEGIN_SRC elisp
    (if (equal "markus" user-login-name)
        (setq user-mail-address "markussievers88@gmail.com")
      (setq user-mail-address "markus.sievers@k-ai.at"))
  #+END_SRC
